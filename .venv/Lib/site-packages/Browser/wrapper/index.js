...
async function getElements(request2, state2) {
  ...
  logger4.info(`Wait element to reach attached state.`);
  try {
    await locator.first().waitFor({ state: "attached" });
  } catch (e) {
    logger4.debug(`Attached state not reached, suppress error: ${e}.`);
  }
  const allLocators = await locator.all();
  logger4.info(`Found ${allLocators.length} elements.`);
...
async function getByX(request2, state2) {
  ...
  switch (strategy) {
    ...
    case "Role": {
      ...
      locator = activePage.getByRole(text, options);
      break;
    }
    case "TestId": {
...
async function waitForFunction(request2, state2, page) {
  ...
  const options = JSON.parse(request2.getOptions());
  const strictMode = request2.getStrict();
  logger4.info(`unparsed args: ${request2.getScript()}, ${request2.getSelector()}, ${request2.getOptions()}`);
  let elem;
  if (selector) {
    const locator = await findLocator(state2, selector, strictMode, void 0, true);
...
async function highlightAll(selector, duration, width, style, color, strictMode, state2) {
  ...
  logger4.info(`Locator count is ${count}`);
...
async function download(request2, state2) {
  ...
  const path2 = await (await downloadStarted).path();
  const fileName = (await downloadStarted).suggestedFilename();
  logger4.info("Suggested file name: " + fileName + " and save as path: " + path2);
  return jsonResponse(
    JSON.stringify({ saveAs: path2, suggestedFilename: fileName }),
    "Url content downloaded to a file: " + path2
  );
}
...
async function getSelections(locator) {
  ...
  if (selectOptions) {
    ...
    logger5.info(`Option entries: ${entries.length}`);
    logger5.info(`Selected entries: ${entries.filter((e) => e.getSelected()).length}`);
...
async function getText(request2, state2) {
  ...
  try {
    ...
    logger5.info(`Retrieved text for element ${selector} containing ${content}`);
  } catch (e) {
    if (e instanceof Error) {
      logger5.error(e);
    }
...
async function getProperty(request2, state2) {
  ...
  try {
    ...
    logger5.info(`Retrieved dom property for element ${selector} containing ${content}`);
    return content;
  } catch (e) {
    if (e instanceof Error) {
      logger5.error(e);
...
async function getAttributeValue(request2, state2) {
  ...
  logger5.info(`Retrieved attribute for element ${selector} containing ${attribute}`);
  return attribute;
}
var stateEnum = {
  attached: 1,
...
async function getStyle(request2, state2) {
  ...
  const strictMode = request2.getStrict();
  logger5.info("Getting css of element on page");
  const locator = await findLocator(state2, selector, strictMode, void 0, true);
...
var import_pino6 = __toESM(require("pino"));
var logger6 = (0, import_pino6.default)({ timestamp: import_pino6.default.stdTimeFunctions.isoTime });
async function selectOption(request2, state2) {
  ...
  if (result.length == 0) {
    logger6.info("Couldn't select any options");
...
async function internalClick(selector, strictMode, options, state2) {
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith("locator.click: Target closed")) {
      logger6.warn("Supress locator.click: Target closed error");
      return false;
    }
    throw error;
  }
...
async function uploadFileBySelector(request2, state2) {
  const selector = request2.getSelector();
  const strictMode = request2.getStrict();
  const path2 = request2.getPath();
  const locator = await findLocator(state2, selector, strictMode, void 0, true);
  await locator.setInputFiles(path2);
...
async function handleAlert(request2, page) {
  ...
  const fn = async (dialog) => {
    ...
    logger6.info(`Alert text: ${dialogueText}`);
  };
  page.on("dialog", fn);
...
async function waitForDownload(request2, page) {
  const saveAs = request2.getPath();
  const downloadObject = await page.waitForEvent("download");
  if (saveAs) {
    await downloadObject.saveAs(saveAs);
  }
  const path2 = await downloadObject.path();
  const fileName = downloadObject.suggestedFilename();
  logger7.info("suggestedFilename: " + fileName + " saveAs path: " + path2);
  return jsonResponse(
    JSON.stringify({ saveAs: path2, suggestedFilename: fileName }),
    "Download done successfully to: " + path2
  );
}
...
async function _connectBrowser(browserType, url) {
  browserType = browserType || "chromium";
  let browser;
  if (browserType === "firefox") {
    browser = await import_playwright4.firefox.connect({ wsEndpoint: url });
  } else if (browserType === "chromium") {
    browser = await import_playwright4.chromium.connect({ wsEndpoint: url });
  } else if (browserType === "webkit") {
    browser = await import_playwright4.webkit.connect({ wsEndpoint: url });
  } else {
    throw new Error("unsupported browser");
  }
  return {
...
var BrowserState = class {
  constructor(browserAndConfs) {
    ...
    this._contextStack = [];
    this.id = `browser=${(0, import_uuid.v4)()}`;
  }
  async close() {
...
async function closePage(openBrowsers) {
  const activeBrowser = openBrowsers.getActiveBrowser();
...
async function newPage(request2, openBrowsers) {
  ...
  const url = request2.getUrl() || "about:blank";
  try {
...
async function _switchPage(id, browserState) {
  ...
  logger8.info("Changing current active page");
...
var PlaywrightServer = class {
  constructor() {
    ...
    this.closePage = this.wrappingState(closePage);
    ...
    this.waitForDownload = this.wrappingPage(waitForDownload);
